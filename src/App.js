...
const App=()=>{
  ...
  const addEvents=(obj)=>{
    ...
    if (indx!==-1){
       ...
       setOriginal(newArr)
        ...
        setOriginal(preEvents=>[...preEvents,obj])
    }
  ...
  const filterEvents=(fltr)=>{
     ...
     if (events.length>0){
      const filteredEvents=original.filter((i)=>i.category===fltr)
  ...
  const delEvents=(identifier)=>{
     ...
     const oriEvents=original.filter((i)=>i.id!==identifier)
     setOriginal(oriEvents)
     setEvents(modEvents)
  }
